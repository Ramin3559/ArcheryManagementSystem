...
public static class AdminCustomersExcelExporter
{
    public static byte[] Export(IReadOnlyCollection<CustomerListItem> rows)
    {
        ...
        foreach (var x in rows)
        {
            ...
            ws.Cell(r, 15).Value = x.DeletedAtLocal ?? "";
            ws.Cell(r, 16).Value = x.DeletedByName;
            ws.Cell(r, 17).Value = x.LastLaneVisitLocal ?? "";
            ws.Cell(r, 18).Value = x.LastLaneNumber is int ln ? ln : "";
            ws.Cell(r, 19).Value = FormatSessionRentalLabel(x);
            ws.Cell(r, 20).Value = x.ActiveLaneNumber is int aln ? $"Zolaq {aln}" : "";
            r++;
        }
